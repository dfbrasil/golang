...
//ValidarTokne verificar se o token passado na requisição é válido
...
//ExtrairUsuarioId retorna o usuario que está salvo no token
func ExtrairUsuarioID(r *http.Request) (uint64, error){
	...
	if permissoes, ok := token.Claims.(jwt.MapClaims); ok && token.Valid{
		usuarioID, erro := strconv.ParseUint(fmt.Sprintf("%.0f", permissoes["usuarioID"]),10, 64) //Pdde ser problema esse IDcom letra maiuscula
		if erro != nil{
			return 0, erro
		}
